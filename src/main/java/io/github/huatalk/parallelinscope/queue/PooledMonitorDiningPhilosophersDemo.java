...
public final class PooledMonitorDiningPhilosophersDemo {
    ...
    private static void pause(String action, int philosopherId, int meal)
    ...
    private static final class DiningTable {
        ...
        private void pickUpForks(int taskId, int philosopherId, int meal)
                throws InterruptedException {
            ...
            System.out.printf(
                    "Philosopher %d acquired forks %d and %d for meal %d "
                            + "(active eaters: %d).%n",
                    displayId(philosopherId),
                    left.id,
                    right.id,
        ...
        private void putDownForks(int taskId, int philosopherId, int meal) {
            ...
            first.release(taskId, philosopherId);
            System.out.printf(
                    "Philosopher %d finished meal %d and released forks %d and %d "
                            + "(active eaters: %d).%n",
                    displayId(philosopherId),
                    meal,
                    left.id,
            ...
            try {
                if (owner != taskId) {
                    throw new IllegalStateException(
                            "Philosopher " + (philosopherId + 1) + " does not own fork " + id);
                }
                owner = -1;
            } finally {
