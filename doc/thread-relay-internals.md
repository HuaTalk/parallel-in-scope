# ThreadRelay 内部机制：两层 Map 如何实现零拷贝上下文传播

在结构化并发中，父线程提交任务到线程池后，子线程需要感知父线程的上下文——取消令牌、执行选项、任务名称。如果每个任务都手动传递这些参数，API 会迅速膨胀。parallel-in-scope 的 `ThreadRelay` 用一种"两层 Map"设计解决了这个问题：**读取穿透到父上下文，写入留在本地，整个过程不需要拷贝任何数据**。
...
parallel-in-scope 的核心 API 是 `Par.forEach()` 和 `Par.map()`。调用者在父线程中配置 `CancellationToken`、`ParOptions`，然后框架将任务分发到线程池执行。问题在于：
...
阿里的 `TransmittableThreadLocal` (TTL) 解决了这个问题。它通过包装 `Runnable`/`Callable`，在任务提交时捕获父线程的值，在 Worker 线程执行时回放。但这只是解决了"值能传过来"的问题——**如何组织这些值，让读写语义正确，才是 ThreadRelay 要解决的核心问题**。
...
关键点：**子任务通过 `curMap` 覆盖了父级的值，但 `parentMap` 中的原始值仍然可读**。这实现了"继承但可覆盖"的语义。

## 为什么是"零拷贝"

严格来说，`parentMap.putAll(parentContext)` 确实复制了 Map 的条目。但这里的"零拷贝"指的是**值对象层面**——`CancellationToken`、`ParOptions` 这些对象本身不会被深拷贝，`parentMap` 和父线程的 `curMap` 持有相同的对象引用。
...
对于 `ConcurrentHashMap` 中存储的不可变或线程安全对象（`CancellationToken` 内部用 `AtomicBoolean`，`ParOptions` 是不可变对象），共享引用是安全的。
...
|------|--------------|---------------|
| 传播性 | 不传播（普通 `ThreadLocal`） | 传播（通过 TTL） |
| 用途 | 当前任务执行期间的临时状态 | 跨线程的上下文接力 |
| 清理 | `finally` 块中 `remove()` | `curMap` 随任务结束清理 |
...
**选择 `ConcurrentHashMap` 而非普通 `HashMap`**：`RelayItem` 枚举只有 4 个值，理论上用 `EnumMap` 更高效。但 `EnumMap` 不是线程安全的，而 `parentMap` 可能被多线程读取（虽然在当前设计中 `ThreadRelay` 是线程局部的，但防御性编程选择了 `ConcurrentHashMap`）。

**`putAll` 的开销**：每次 TTL 传播都会 `putAll` 4 个条目。对于百万级任务提交，这是可测量但可忽略的开销——远小于任务本身的执行成本。

**不提供 `getParent*` 的穿透查询**：`getParentCancellationToken()` 只查 `parentMap`，不回退到 `curMap`。这保持了语义清晰——"parent"就是 parent，不会意外读到自己设置的值。

**`curMap` 不主动清理 `parentMap`**：子任务可以覆盖父级的值写入 `curMap`，但不会修改 `parentMap`。这保证了兄弟任务之间互不干扰——每个子任务看到的 `parentMap` 都是父线程 `curMap` 的同一份快照。
