...
关键规则：
- **第一个参数必须与 `ExecutionOptions.of(taskName)` 中的 taskName 一致**。这是一个安全守卫——checkpoint 只在 taskName 匹配时才生效，防止被不相关的代码误触发。
...
| 超时 | `TIMEOUT_CANCELED` | 超过 `ExecutionOptions` 指定的超时时间 |
