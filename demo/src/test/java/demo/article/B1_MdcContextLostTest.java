...
public class B1_MdcContextLostTest {
  ...
  void parMap_executesAllTasksWithCleanResult() throws Exception {
    // 设置并行选项
    ParOptions opts = ParOptions.of("mdc-demo").parallelism(3).timeout(5000).build();
   ...
   *
   * <p>框架内部使用 TransmittableThreadLocal 传播 CancellationToken、 ParOptions、taskName
  ...
  void parMap_frameworkContextAutoPropagated() throws Exception {
    ParOptions opts = ParOptions.of("ttl-verify").parallelism(3).timeout(5000).build();
    ...
    assertThat(taskNames)
        .containsExactlyInAnyOrder("order-1", "order-2", "order-3", "order-4", "order-5");
  }
}
