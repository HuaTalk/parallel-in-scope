...
public class ScopedCallable<V> implements Callable<V> {
  ...
  public ScopedCallable(
    ...
    this.newTaskListeners =
        taskListeners == null ? java.util.Collections.emptyList() : taskListeners;
  ...
  public V call() throws Exception {
    ...
    try {
      ...
      ThreadRelay.restoreCurrent(
          previousRelayToken, previousTaskName, previousExecutorName, previousIdentity);
  ...
  private void notifyListeners(Throwable exception) {
    ...
    for (TaskListener listener : listeners) {
      try {
        ...
        logger.log(
            Level.WARNING, "TaskListener callback failed: " + listener.getClass().getName(), e);
      }
    }
  }
