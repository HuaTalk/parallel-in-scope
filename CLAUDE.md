...
- **Maven coordinates:** `io.github.huatalk:parallel-in-scope:0.1.0`
...
6. **`ConcurrentLimitExecutor.submitAll()`** — sliding-window submission through `ListenableCompletionService`, whose returned `FutureRunnable` is the exact queued object
...
| SPI | `TaskListener`, `LivelockListener` | `javax.annotation.Nullable` (JSR-305) |
