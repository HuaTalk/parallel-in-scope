...
大多数框架在创建 Future 时就绑定超时和取消。parallel-in-scope 偏要等所有任务提交完毕才绑定。
...
```java
// 阶段 1：滑动窗口提交所有任务
// 前 parallelism 个任务立即提交，剩余用 SettableFuture 占位
// 每完成一个任务，从队列中取下一个占位符，用 SettableFuture.setFuture() 补充实际任务
ConcurrentLimitExecutor.submitAll(wrappedTasks, parallelism);

// 阶段 2：所有任务入队后，绑定超时、fail-fast、父级取消
cancellationToken.lateBind(futures, timeout);
```

> 注：以上是简化示意。实际实现通过 `ExecutorCompletionService` + `SettableFuture` 占位 + 独立的 `submitterPool` 阻塞循环完成滑动窗口调度，详见 `ConcurrentLimitExecutor.submitAll()` 源码。`lateBind()` 实际还有一个 `submitCanceller` 参数，用于在提交阶段被取消时终止后续任务提交。
...
**晚绑定的本质是显式分离"提交阶段"和"绑定阶段"。** 提交阶段只关心滑动窗口的调度逻辑，绑定阶段只关心取消和超时的语义。两个阶段各自独立，不会互相干扰。

> **提交是提交，绑定是绑定。不要在上菜之前开酒。** 先把所有菜上齐（任务全部提交），再开酒（绑定超时和取消）。
...
public boolean offer(E e) {
    ...
    return delegate.offer(o);  // 组合模式，委托给内部队列
}
```

`offer()` 返回 `false` 会触发 `ThreadPoolExecutor` 的 `RejectedExecutionHandler`（通常配置为 `CallerRunsPolicy`），让调用方线程同步执行该任务。

`SmartBlockingQueue` 的策略是：CPU 任务不入队，直接在调用方线程同步执行。这样 IO 任务可以正常入队、正常出队、正常完成。
...
通过 Alibaba TTL（TransmittableThreadLocal）的 `Transmitter.registerThreadLocal()` 注册，子线程创建时自动将父线程的 `curMap` 变为子线程的 `parentMap`。传播的内容包括：`CancellationToken`、`ParOptions`、任务名称、执行器名称。
...
**上下文传播应该是透明的——业务代码不应该知道自己在被并行执行。** 两 Map 接力让并发成为实现细节，而不是侵入性的架构约束。
...
**替代方案：** 在任务函数内部用专门的重试库（Guava `Retryer`、Resilience4j、Spring Retry）。
