...
public class B2_FunctionSignatureBloatTest {
  ...
  private static String fetchWithBloat(
      String url, String traceId, long timeoutMs, boolean[] cancelFlag) {
    // 手动检查取消
    if (cancelFlag[0]) {
      return "CANCELED";
    }
  ...
  void bloatedSignature_requiresPlumbingParameters() {
    ...
    List<String> results =
        batchFetchWithBloat(urls, traceId, "user-1", "org-1", timeoutMs, cancelFlag, 3);
  ...
  void parMap_cleanSignature_onlyBusinessParam() throws Exception {
    ...
    ParOptions opts = ParOptions.of("fetch-data").parallelism(3).timeout(5000).build();
    ...
    AsyncBatchResult<String> result =
        par.map(
            "test-pool",
            urls,
            url -> {
  ...
  void parMap_multipleContextParams_allHandledImplicitly() {
    ...
    ParOptions opts = ParOptions.of("complex-task").parallelism(2).timeout(3000).build();
